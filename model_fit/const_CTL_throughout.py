...
date:     05/02/2013
content:  fitting on simulation data––constant CTL pressure throughout and no noise.
'''
...
for ri in xrange(nrun):
    ...
    fit_coeff[-1][:,0]= f*(1+0.0*np.random.randn(f.shape[0]))
...
plt.ylim([0,0.7])
plt.legend()
plt.savefig('true_vs_estimated_const_throughout_no_noise.pdf')
...
ax=plt.gca()
ax.set_yscale('log')
plt.savefig('t50_vs_estimated_const_throughout_no_noise.pdf')
