...
date:     05/02/2013
content:  performs fitting on simulation dataÐÐwhere selection starts at random times.
'''
...
def allele_frequencies(f,N, mu,r,tmax):
    ...
    CTL_onsets = np.asarray(np.random.rand(f.shape[0])*tmax-100,dtype=int)
    print CTL_onsets
    pop.set_fitness_additive(f*0.5*(CTL_onsets>pop.generation))     #set epitope strength
...
def fit_logistic(tp,mut_count,sample_size):
    ...
    if (np.sum(freq)==0): freq[-1]=1.0/sample_size
    elif (np.sum(freq>0)==1): freq[-2]=1.0/sample_size
    elif (np.sum(freq==1)+np.sum(freq==0) == len(freq)):
...
plt.title('estimated vs true escape rates -- all equal, selection starts at random times')
for ri in xrange(nrun):
...
plt.ylim([0,0.7])
plt.legend()
plt.savefig('true_vs_estimated_const_staggered.png')

plt.figure()
plt.xlabel(r'$t_{50}$')
plt.ylabel(r'estimated $\epsilon$')
plt.title('estimated escape rate vs t50 -- all equal, selection starts at random times')
for ri in xrange(nrun):
...
ax=plt.gca()
ax.set_yscale('log')
plt.savefig('t50_vs_estimated_const_staggered.png')
