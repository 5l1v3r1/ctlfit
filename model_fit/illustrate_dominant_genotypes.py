...
for locus in xrange(L):
    ii = np.array(map(list,map(bin,2**L+np.arange(2**L))))[:,-1-locus]=='1'
    af = np.sum(all_gt_traj[:,ii], axis=1)
    plt.plot(af, c=col[locus+1], label = 'Epitope '+str(locus+1))
    plt.plot(default_params['tp'], 1.0*np.sum(gt[:,(locus+1):],axis=1)/sample_size, ls='None',marker='o', c=col[locus+1], markersize=20)

plt.xlabel(r'time ($\textrm{days}$)')
plt.ylabel('mutation frequencies')
fig.text(0.03,0.93,'A', fontsize=48)
axes.set_xlim([0, tp[-1]*1.05])
axes.set_ylim([0, 1.05])
plt.legend(loc=4)
plt.show()
plt.savefig('figures/mutation_frequency_demonstration.pdf')
plt.savefig('figures/mutation_frequency_demonstration.svg')
...
del fig.texts[-1]
fig.text(0.03,0.93,'C', fontsize=48)
