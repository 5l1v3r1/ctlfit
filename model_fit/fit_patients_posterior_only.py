...
if len(gts)>0:
    ...
    LHtrace = np.array(ctl.MCMC_LH)

    #dump the posterior data
    with open(outdir+'/'+patient+'_F_'+str(F)+'_S_'+str(S)+'_tau_'+str(tau)+'_mu_'+str(mu)+'_logN_'+str(int(np.log10(N)))+'_posterior.pickle', 'w') as pfile:
        pickle.dump((Fsample, STsample, LHtrace,(N,tau, mu)), pfile)
